...
deaths <- deaths_0 %>% mutate(cause = as.character(Multiple.Cause.of.death),
  ...
  filter(code %in% c("T37.4", "T38.3", "T39.0", "T39.1", "T39.3", "T39.8", "T40.1", "T40.2", "T40.3", "T40.4", "T40.5", "T40.6", 
  ...
  group_by(state, year) %>% summarise(opioid_d_all = sum(subset(rate, code %in% c("T40.1", "T40.2", "T40.3", "T40.4"))),
                                      opioid_d_illicit = sum(subset(rate, code %in% c("T40.1"))),
                                      opioid_d_prescription = sum(subset(rate, code %in% c("T40.2", "T40.3", "T40.4"))),
                                      drugs_d_all = sum(rate),
                                      drugs_d_illicit = sum(subset(rate, code %in% c("T40.1", "T40.5", "T40.7", 
                                                                                    "T40.8", "T40.9", "T43.6")))) %>% 
  data.frame()
...
med_disp_1 <- gather(med_disp_neighbors, year, med_neighbors,names(med_disp_neighbors)[2:ncol(med_disp_neighbors)])
rec_disp_1 <- gather(rec_disp_neighbors, year, rec_neighbors,names(rec_disp_neighbors)[2:ncol(rec_disp_neighbors)])
...
states_after_2010 <- tolower(c((leg %>% filter(medical>=2010) %>% select(state))[[1]]))

m = trunc(mean(c((leg %>% filter(medical>=2010) %>% select(medical))[[1]])))

diff$status <- if_else(diff$state %in% selected_states[[1]], 1, 0)

for(i in 1:51) {
  if(diff$status[i] == 1) {
    b <- na.omit(panel_data_clear %>% subset(med == 0 & state == diff$state[i]) %>% select(drugs_d_all, opioid_d_all))
    c <- na.omit(panel_data_clear %>% subset(med == 1 & state == diff$state[i]) %>% select(drugs_d_all, opioid_d_all))
  } else {
    b <- na.omit(panel_data_clear %>% subset(year <= m & state == diff$state[i]) %>% select(drugs_d_all, opioid_d_all))
    c <- na.omit(panel_data_clear %>% subset(year >= m & state == diff$state[i]) %>% select(drugs_d_all, opioid_d_all))
  }
  diff$mean_mort_0[i] <- mean(b$drugs_d_all)
  diff$mean_mort_1[i] <- mean(c$drugs_d_all)
  diff$mean_opioid_mort_0[i] <- mean(b$opioid_d_all)
  diff$mean_opioid_mort_1[i] <- mean(c$opioid_d_all)
}
...
#модели метода dd для зависимой drugs_d_all, opioid_d_all и регрессора rec
...
for(i in 1:51){
  ...
  if(diff1$status[i] == 1){
    b <- na.omit(panel_data_clear%>%subset(rec == 0 & state == diff1$state[i]& year>=2010)%>%select(drugs_d_all, opioid_d_all))
    c <- na.omit(panel_data_clear%>%subset(rec == 1 & state == diff1$state[i])%>%select(drugs_d_all, opioid_d_all))
  } else {
    b <- na.omit(panel_data_clear%>%subset(year <= m1 & year>=2010 & state == diff1$state[i])%>%select(drugs_d_all, opioid_d_all))
    c <- na.omit(panel_data_clear%>%subset(year >= m1 & state == diff1$state[i])%>%select(drugs_d_all, opioid_d_all))
  }
  diff1$mean_mort_0[i] <- mean(b$drugs_d_all)
  diff1$mean_mort_1[i] <- mean(c$drugs_d_all)
  diff1$mean_opioid_mort_0[i] <- mean(b$opioid_d_all)
  diff1$mean_opioid_mort_1[i] <- mean(c$opioid_d_all)
}
