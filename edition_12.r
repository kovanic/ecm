...
deaths <- deaths_0 %>% mutate(cause = as.character(Multiple.Cause.of.death),
  ...
  filter(code %in% c("X40", "X41", "X42", "X43", "X44", "X60", "X61", "X62", "X63", "X64", "X85",
                     "Y10", "Y11", "Y12", "Y13", "Y14", "T40.1", "T40.2", "T40.3", "T40.4", "T43.6")) %>%
  group_by(state, year) %>% summarise(death_rate = sum(rate)) %>% data.frame()

#codes_out <- deaths_0 %>% select(3,4) %>% 
#  filter(!(code %in% c("X40", "X41", "X42", "X43", "X44", "X60", "X61", "X62", "X63", "X64", "X85",
...
panel_data <- full_join(overdose, legal, by=c("state", "year")) %>%
  ...
  full_join(deaths_3, by=c("state", "year")) %>% full_join(deaths, by=c("state", "year")) %>%
...
# Graphic analysis
crime_plot <- panel_data_clear %>% gather(crime, rate, violent_crime_on100k:property_crime_on100k) %>%
...
medplot2 <- panel_data_clear %>% 
  ggplot(aes(x=factor(rec), y=alldrugs_d_adj)) + geom_boxplot() +
  ggtitle("Различный разброс данных по передозировкам") +
...
corr <- cor(panel_data_clear %>%
              select(-c("state", "population", "year", "total_crime_on100k", "percent_male", "rec")) %>%
              drop_na()
            )

correlation <- ggcorrplot(corr, lab = FALSE, lab_size = 5, digits = 1) + 
  theme(axis.text.x = element_text(color = "grey20", size = 10), 
        axis.text.y = element_text(color = "grey20", size = 10), 
        text = element_text(size = 10))
...
# names(panel_data_clear)[-c(1,2)]

# "opioid_d_adj"          "alldrugs_d_adj"        "med"                   "rec"                   "pdmp"                 
# "violent_crime_on100k"  "property_crime_on100k" "total_crime_on100k"    "gdp_per_cap"           "income_per_cap"       
# "unemployment"          "population"            "age_0_14"              "age_15_24"             "age_25_44"            
# "age_45_59"             "age_60_more"           "northeast"             "midwest"               "south"                
# "west"                  "alco_consumption"      "poverty"               "percent_male"          "percent_black"        
# "percent_hisp_origin"   "marriage_rate"         "divorce_rate"          "total_d"               "alco_d"               
# "other_d"               "drug_d"                "homicide"              "suicide"               "drug_other_causes"    
# "drug_homicide"         "drug_suicide"          "drug_undetermined"     "drug_unintentional"    "death_rate"           
# "med_neighbors"         "rec_neighbors"         "prescr_rate"    
...
# opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + income_per_cap + homicide + alco_consumption
reg1 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + income_per_cap + homicide + alco_consumption,
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg1)

reg2 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + income_per_cap + homicide + alco_consumption +
              factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg2)

reg3 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + income_per_cap + homicide + alco_consumption,
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg3)

reg4 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + income_per_cap + homicide + alco_consumption +
              factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg4)

reg5 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + income_per_cap + homicide + alco_consumption,
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg5)

reg6 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + income_per_cap + homicide + alco_consumption +
              factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg6)

stargazer(reg1, reg2, reg3, reg4, reg5, reg6,
          se = list(clse(reg1), clse(reg2), clse(reg3), clse(reg4), clse(reg5), clse(reg6)), 
          title = "Panel regressions, clustered SEs", type="text", 
          column.labels = c("Pooled", "Pooled+time", "FE", "FE+time", "RE", "RE+time"), 
...
# 
reg1 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption,
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg1)

reg2 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg2)

reg3 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption,
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg3)

reg4 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg4)

reg5 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption,
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg5)

reg6 <- plm(opioid_d_adj ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg6)
...
reg1 <- plm(drug_d ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + percent_male + percent_black + percent_hisp_origin +
              age_0_14 + age_15_24 + age_25_44 + age_45_59,
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg1)

reg2 <- plm(drug_d ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + percent_male + percent_black + percent_hisp_origin + factor(year) +
              age_0_14 + age_15_24 + age_25_44 + age_45_59,
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg2)

reg3 <- plm(drug_d ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + percent_male + percent_black + percent_hisp_origin +
              age_0_14 + age_15_24 + age_25_44 + age_45_59,
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg3)

reg4 <- plm(drug_d ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + percent_male + percent_black + percent_hisp_origin + factor(year) +
              age_0_14 + age_15_24 + age_25_44 + age_45_59,
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg4)

reg5 <- plm(drug_d ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + percent_male + percent_black + percent_hisp_origin +
              age_0_14 + age_15_24 + age_25_44 + age_45_59,
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg5)

reg6 <- plm(drug_d ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + percent_male + percent_black + percent_hisp_origin + factor(year) +
              age_0_14 + age_15_24 + age_25_44 + age_45_59,
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg6)
...
##################################################################################################################################
# death_rate ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest + 
# + income_per_cap + homicide + alco_consumption + prescr_rate
reg1 <- plm(death_rate ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest + 
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg1)

reg2 <- plm(death_rate ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg2)

reg3 <- plm(death_rate ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg3)

reg4 <- plm(death_rate ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg4)

reg5 <- plm(death_rate ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg5)

reg6 <- plm(death_rate ~ med + rec + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg6)


# death_rate ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest + 
# + income_per_cap + homicide + alco_consumption + prescr_rate
reg1 <- plm(death_rate ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest + 
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg1)

reg2 <- plm(death_rate ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg2)

reg3 <- plm(death_rate ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg3)

reg4 <- plm(death_rate ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg4)

reg5 <- plm(death_rate ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg5)

reg6 <- plm(death_rate ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg6)
...
# opioid_d_adj ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest + 
# + income_per_cap + homicide + alco_consumption + prescr_rate
reg1 <- plm(opioid_d_adj ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest + 
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg1)

reg2 <- plm(opioid_d_adj ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "pooling", effect = "individual")
summary(reg2)

reg3 <- plm(opioid_d_adj ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg3)

reg4 <- plm(opioid_d_adj ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "within", effect = "individual")
summary(reg4)

reg5 <- plm(opioid_d_adj ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate,
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg5)

reg6 <- plm(opioid_d_adj ~ med + rec + pdmp + med_neighbors + rec_neighbors + unemployment + west + south + midwest +
              income_per_cap + homicide + alco_consumption + prescr_rate + factor(year),
            data = panel_data_clear, index = c("state", "year"),  model = "random", effect = "individual")
summary(reg6)
