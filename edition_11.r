...
panel_data <- full_join(overdose, legal, by=c("state", "year")) %>%
  ...
  full_join(deaths_3, by=c("state", "year")) %>% full_join(med_1, by=c("state", "year")) %>%
...
corr <- cor(panel_data_clear %>%
             select(-c("state", "population", "year", "total_crime_on100k", 
                       "rec", "percent_male")) %>%
             drop_na())

correlation <- ggcorrplot(corr, lab = TRUE, lab_size = 5, digits = 1) + 
  theme(axis.text.x = element_text(color = "grey20", size = 10), 
        axis.text.y = element_text(color = "grey20", size = 10), 
        text = element_text(size = 10))
...
# "opioid_d_adj"            "alldrugs_d_adj"          "med"                   "rec"                   
# "violent_crime_on100k"    "property_crime_on100k"   "total_crime_on100k"    "gdp_per_cap"            
# "income_per_cap"          "unemployment"            "population"            "age_0_14"               
# "age_15_24"               "age_25_44"               "age_45_59"             "age_60_more"            
# "northeast"               "midwest"                 "south"                 "west"                   
# "alco_consumption"        "poverty"                 "percent_male"          "percent_black"          
# "percent_hisp_origin"     "marriage_rate"           "divorce_rate"          "total_d"                
# "alco_d"                  "other_d"                 "drug_d"                "homicide"               
# "suicide"                 "drug_other_causes"       "drug_homicide"         "drug_suicide"           
# "drug_undetermined"       "drug_unintentional"      "med_neighbors"         "rec_neighbors"   
