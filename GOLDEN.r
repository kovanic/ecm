...
panel_data_clear <- read.csv("panel.csv")[, 2:45]
...
# Пригодистя для bootstrap, в следующей модели используем этот датасет (очищен от NA)
panel_data_modeling_1 <- panel_data_clear %>% drop_na(drugs_d_all, med, rec, pdmp, total_crime_on100k, income_per_cap, 
...
reg1 <- plm(drugs_d_all ~ med + rec + pdmp + total_crime_on100k + income_per_cap + alco_consumption + 
              unemployment + age_0_14 + age_15_24 + age_45_59 + age_25_44 + northeast + midwest + south +
              percent_male + percent_black + percent_hisp_origin,
            data = panel_data_modeling_1, index = c("state", "year"),
            model = "within", effect = "individual")
summary(reg1)

# Гистограмма остатков первой модели
hist(reg1$residuals, breaks = 100)
# Находим спрогнозированные игрики
predictions <- panel_data_modeling_1$drugs_d_all - reg1$residuals
# сэмплируем остатки
boots <- bootstrap(data.frame(reg1$residuals), 1000)
coefs1 <- reg1$coefficients
for (i in 1:1000) {
  panel_data_modeling_1$drugs_d_all_m <- predictions + data.frame(boots$strap[[i]])$reg1.residuals
  r <- plm(drugs_d_all_m ~ med + rec + pdmp + total_crime_on100k + income_per_cap + alco_consumption +
             unemployment + age_0_14 + age_15_24 + age_45_59 + age_25_44 + northeast + midwest + south + 
             percent_male + percent_black + percent_hisp_origin,
           data = panel_data_modeling_1, index = c("state", "year"),
           model = "within", effect = "individual")
  coefs1 <- rbind(coefs1, r$coefficients)
}

# Смотрим на коэффициенты модели исходной
coefs1[1, ]
# И смотрим на квантили (изучаем, устойчивы ли результаты)
q1 <- apply(coefs1[2:1001, ], MARGIN = 2, FUN = quantile, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
q1


# Пригодистя для bootstrap, в следующей модели используем этот датасет (очищен от NA)
panel_data_modeling_2 <- panel_data_clear %>% drop_na(drugs_d_all, med, rec, pdmp, med_neighbors, rec_neighbors, 
                                                      total_crime_on100k, income_per_cap, alco_consumption, unemployment, 
                                                      age_0_14, age_15_24, age_45_59, age_25_44, northeast, midwest, south,
                                                      percent_male, percent_black, percent_hisp_origin)

reg2 <- plm(drugs_d_all ~ med + rec + pdmp + med_neighbors + rec_neighbors + total_crime_on100k + 
              income_per_cap + alco_consumption + unemployment + age_0_14 + age_15_24 + age_45_59 + age_25_44 +
              northeast + midwest + south + percent_male + percent_black + percent_hisp_origin,
            data = panel_data_modeling_2, index = c("state", "year"),
            model = "within", effect = "individual")
summary(reg2)

# Гистограмма остатков второй модели
hist(reg2$residuals, breaks = 100)
# Находим спрогнозированные игрики
predictions <- panel_data_modeling_2$drugs_d_all - reg2$residuals
# сэмплируем остатки
boots <- bootstrap(data.frame(reg2$residuals), 1000)
coefs2 = reg2$coefficients
for (i in 1:1000) {
  panel_data_modeling_2$drugs_d_all_m <- predictions + data.frame(boots$strap[[i]])$reg2.residuals
  r <- plm(drugs_d_all_m ~ med + rec + pdmp + total_crime_on100k + income_per_cap + alco_consumption +
             unemployment + age_0_14 + age_15_24 + age_45_59 + age_25_44 + northeast + midwest + south + 
             percent_male + percent_black + percent_hisp_origin,
           data = panel_data_modeling_2, index = c("state", "year"),
           model = "within", effect = "individual")
  coefs2 <- rbind(coefs2, r$coefficients)
}

# Смотрим на коэффициенты модели исходной
coefs2[1, ]
# И смотрим на квантили (изучаем, устойчивы ли результаты)
q2 <- apply(coefs2[2:1001, ], MARGIN = 2, FUN = quantile, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
q2
...
# Пригодистя для bootstrap, в следующей модели используем этот датасет (очищен от NA)
panel_data_modeling_3 <- panel_data_clear %>% drop_na(opioid_d_all, med, rec, pdmp, total_crime_on100k, income_per_cap, 
                                                      alco_consumption, unemployment, age_0_14, age_15_24, age_45_59, 
                                                      age_25_44, northeast, midwest, south, percent_male, percent_black, 
                                                      percent_hisp_origin)
...
reg3 <- plm(opioid_d_all ~ med + rec + pdmp + total_crime_on100k + income_per_cap + alco_consumption + 
              unemployment + age_0_14 + age_15_24 + age_45_59 + age_25_44 + northeast + midwest + south +
              percent_male + percent_black + percent_hisp_origin,
            data = panel_data_modeling_3, index = c("state", "year"),
            model = "within", effect = "individual")
summary(reg3)

# Гистограмма остатков третьей модели
hist(reg3$residuals, breaks = 100)
# Находим спрогнозированные игрики
predictions <- panel_data_modeling_3$opioid_d_all - reg3$residuals
# сэмплируем остатки
boots <- bootstrap(data.frame(reg3$residuals), 1000)
coefs3 = reg3$coefficients
for (i in 1:1000) {
  panel_data_modeling_3$opioid_d_all_m <- predictions + data.frame(boots$strap[[i]])$reg3.residuals
  r <- plm(opioid_d_all_m ~ med + rec + pdmp + total_crime_on100k + income_per_cap + alco_consumption +
             unemployment + age_0_14 + age_15_24 + age_45_59 + age_25_44 + northeast + midwest + south + 
             percent_male + percent_black + percent_hisp_origin,
           data = panel_data_modeling_3, index = c("state", "year"),
           model = "within", effect = "individual")
  coefs3 <- rbind(coefs3, r$coefficients)
}

# Смотрим на коэффициенты модели исходной
coefs3[1, ]
# И смотрим на квантили (изучаем, устойчивы ли результаты)
q3 <- apply(coefs3[2:1001, ], MARGIN = 2, FUN = quantile, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
q3


# Пригодистя для bootstrap, в следующей модели используем этот датасет (очищен от NA)
panel_data_modeling_4 <- panel_data_clear %>% drop_na(opioid_d_all, med, rec, pdmp, med_neighbors, rec_neighbors, 
                                                      total_crime_on100k, income_per_cap, alco_consumption, unemployment, 
                                                      age_0_14, age_15_24, age_45_59, age_25_44, northeast, midwest, south,
                                                      percent_male, percent_black, percent_hisp_origin)
...
reg4 <- plm(opioid_d_all ~ med + rec + pdmp +  med_neighbors + rec_neighbors + total_crime_on100k + 
              income_per_cap + alco_consumption + unemployment + age_0_14 + age_15_24 + age_45_59 + age_25_44 +
              northeast + midwest + south + percent_male + percent_black + percent_hisp_origin,
            data = panel_data_modeling_4, index = c("state", "year"),
            model = "within", effect = "individual")
summary(reg4)

# Гистограмма остатков четвёртой модели
hist(reg4$residuals, breaks = 100)
# Находим спрогнозированные игрики
predictions <- panel_data_modeling_4$opioid_d_all - reg4$residuals
# сэмплируем остатки
boots <- bootstrap(data.frame(reg4$residuals), 1000)
coefs4 <- reg4$coefficients
for (i in 1:1000) {
  panel_data_modeling_4$opioid_d_all_m <- predictions + data.frame(boots$strap[[i]])$reg4.residuals
  r <- plm(opioid_d_all_m ~ med + rec + pdmp +  med_neighbors + rec_neighbors + total_crime_on100k + 
             income_per_cap + alco_consumption + unemployment + age_0_14 + age_15_24 + age_45_59 + age_25_44 +
             northeast + midwest + south + percent_male + percent_black + percent_hisp_origin,
           data = panel_data_modeling_4, index = c("state", "year"),
           model = "within", effect = "individual")
  coefs4 <- rbind(coefs4, r$coefficients)
}

# Смотрим на коэффициенты модели исходной
coefs4[1, ]
# И смотрим на квантили (изучаем, устойчивы ли результаты)
q4 <- apply(coefs4[2:1001, ], MARGIN = 2, FUN = quantile, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
q4
